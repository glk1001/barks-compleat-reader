...
class TestCompress:
    """The publish-time squeeze, and the guard that stops it wrecking the chapters."""

    def test_an_unlisted_output_is_left_alone(self, tmp_path: Path) -> None:
        """demo.mp4 autoplays as the hero and is small; only the walkthrough is squeezed."""
        video = tmp_path / "demo.mp4"
        video.write_bytes(b"master")

        with patch.object(record_demo, "_run_ffmpeg") as mock_ffmpeg:
            record_demo.compress(video)

        mock_ffmpeg.assert_not_called()
        assert video.read_bytes() == b"master"

    def test_a_listed_output_is_replaced_by_the_squeezed_copy(self, tmp_path: Path) -> None:
        video = tmp_path / "walkthrough.mp4"
        video.write_bytes(b"master")

        def fake_ffmpeg(args: list[str]) -> None:
            Path(args[-1]).write_bytes(b"squeezed")

        with (
            patch.object(record_demo, "_run_ffmpeg", side_effect=fake_ffmpeg),
            patch.object(record_demo, "duration_of", return_value=100.0),
        ):
            record_demo.compress(video)

        assert video.read_bytes() == b"squeezed"
        assert not (tmp_path / "walkthrough-compressed.mp4").exists()

    def test_a_drifted_running_time_is_refused(self, tmp_path: Path) -> None:
        """A shifted duration puts every chapter out of step.

        The manifest is derived from the beat clips, so the stitched file is kept and
        the failure said out loud rather than published silently wrong.
        """
        video = tmp_path / "walkthrough.mp4"
        video.write_bytes(b"master")
        durations = iter([100.0, 101.0])  # before, after
        ...
        with (
            patch.object(record_demo, "_run_ffmpeg", side_effect=fake_ffmpeg),
            patch.object(record_demo, "duration_of", side_effect=lambda _p: next(durations)),
            pytest.raises(record_demo.BeatError, match="slide every chapter boundary"),
        ):
            record_demo.compress(video)

        assert video.read_bytes() == b"master"  # untouched
        assert not (tmp_path / "walkthrough-compressed.mp4").exists()

    def test_a_frame_of_drift_is_tolerated(self, tmp_path: Path) -> None:
        video = tmp_path / "walkthrough.mp4"
        video.write_bytes(b"master")
        durations = iter([100.0, 100.05])  # under one frame at 15fps
