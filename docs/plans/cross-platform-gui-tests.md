...
> Status: **approved in discussion 2026-09-23**; steps 1-4 done, and the laptop run. Four steps, each useful on its
...
> - The laptop run: DONE 2026-09-24. The whole suite on the Windows 11 touchscreen laptop,
>   without `--angle`, so through the reader's normal OpenGL drawing (Intel graphics) that
>   neither the VM nor CI covers: 60 passed and 1 skipped (prebuilt archives), in 12m55s,
>   calibrated (`.benchmarks/gui-timings.json`; the slowest comic load 4.8s, inside the
>   committed 6s budget). No app bug on real OpenGL. `test_one_pagers_ignore_double_page` was deselected:
>   the laptop's data pack predates volume 1's `628.json` panel segments, so the reader
>   stops at its error popup there - rerun it alone once the pack is updated. What the runs
>   found, all in the harness or the machine:
>   - **Harness (6c05532c):** a failed test crashed pytest itself - saving its artifacts
>     wrote the log tail in cp1252, which cannot hold the log's box drawing, and hid the
>     test's own failure. Every text read and write in the harness, probe and driver now
>     names UTF-8 (the VM never failed a test, so never saved artifacts).
>   - **Probe (0365752a):** the window was found by title alone, so a Firefox tab named for
>     the app counted - doctor refused a run, and a run could have keyed the browser. Only
>     SDL windows count now.
>   - **Machine setup:** a missing Fantagraphics volume (30, present only as a `.cbr`);
>     no pre-commit or bun, and pre-commit's chaining to the LFS hook broke every push on
>     Windows (7d3301d1, c5c2aedd, 00a53ddd: `CLAUDE.md` has the setup). Two unit tests
>     also failed only on Windows: the wiki viewer's page-shown marker used backslashes,
...
>    step on Windows rather than chase its last ~70ms now: run `--keep-logs -k fullscreen` and look at
