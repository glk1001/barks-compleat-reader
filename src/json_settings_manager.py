...
READER_SETTINGS = "AAA_Settings"
READER_SETTING_LAST_SELECTED_NODE = "last_selected_node"
TITLE_SETTING_LAST_READ_PAGE = "last_read_page"
...
class SettingsManager:
    ...
    def get_last_selected_node_path(self) -> Union[List[str], None]:
        ...
        if not self.store.exists(READER_SETTINGS):
            return None
        raw_path = self.store.get(READER_SETTINGS).get(READER_SETTING_LAST_SELECTED_NODE)
    ...
    def save_last_selected_node_path(self, path: List[str]) -> None:
        ...
        self.store.put(READER_SETTINGS, **{READER_SETTING_LAST_SELECTED_NODE: json.dumps(path)})
    ...
    def get_last_read_page(self, title_str: str) -> Union[SavedPageInfo, None]:
        ...
        if not self.store.exists(title_str):
            return None
        json_info = self.store.get(title_str).get(TITLE_SETTING_LAST_READ_PAGE)
    ...
    def save_last_read_page(self, title_str: str, page_info: SavedPageInfo) -> None:
        ...
        self.store.put(title_str, **{TITLE_SETTING_LAST_READ_PAGE: page_info.to_json()})
