...
class TestReaderUtils(unittest.TestCase):
    ...
    def test_get_best_window_height_fit(self, mock_taskbar_height: MagicMock) -> None:
    ...
    def test_get_title_str_from_reader_icon_file(self) -> None:
        ...
        assert (
            reader_utils.get_title_str_from_reader_icon_file(Path("My-Title-1.png")) == "My-Title"
        )
    ...
    def test_get_image_stream(self, mock_core_image: MagicMock) -> None:
        # Test with Path
    ...
    def test_prob_rand_less_equal(self, mock_randrange: MagicMock) -> None:
        mock_randrange.return_value = 50
        assert reader_utils.prob_rand_less_equal(60)
        assert not reader_utils.prob_rand_less_equal(40)

    @patch("barks_reader.reader_utils.randrange")
    def test_get_rand_int(self, mock_randrange: MagicMock) -> None:
    ...
    def test_get_all_files_in_dir(self) -> None:
    ...
    def test_get_paths_from_directory(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            (root / "file1.txt").touch()
            (root / "subdir").mkdir()
    ...
    def test_get_paths_from_zip(self, mock_zipfile: MagicMock) -> None:
        mock_zip = MagicMock()
    ...
    def test_get_centred_position_on_primary_monitor(self, mock_get_monitors: MagicMock) -> None:
    ...
    def test_safe_import_check(self, mock_run: MagicMock) -> None:
        mock_proc = MagicMock()
