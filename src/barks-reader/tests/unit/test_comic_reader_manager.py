...
from barks_reader.core.comic_book_page_info import ComicBookPageInfo, DisplayUnit, PageInfo
...
class TestComicReaderManager:
    ...
    def test_comic_closed_save(
        ...
        # Setup state — single page mode, but display unit has a right page.
        mock_reader = MagicMock()
    ...
    def test_comic_closed_double_page_with_back_matter(self, manager: ComicReaderManager) -> None:
        ...
        mock_comic_page_info = ComicBookPageInfo(
            required_dim=RequiredDimensions(100, 100),
            page_map=OrderedDict([("9", body_page), ("10", back_matter_page)]),
            last_body_page="9",
            last_page="10",
        )
        manager._comic_page_info = mock_comic_page_info
