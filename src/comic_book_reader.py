...
class _ComicPageManager(EventDispatcher):
    ...
    def __init__(self, current_page_index_bound_func: Callable, *args, **kwargs):
    ...
    def goto_start_page(self) -> None:
        if self._current_page_index == self._first_page_index:
            ...
            logging.debug(f"Goto start page: requested index = 0.")
    ...
    def prev_page(self) -> None:
        if self._current_page_index <= self._first_page_index:
            logging.debug(f"Already on the first page: current index = 0.")
        else:
...
class ComicBookReader(BoxLayout):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._closed = False
        self._goto_page_dropdown: Union[DropDown, None] = None
    ...
    def _show_page(self, _instance, _value):
        ...
        try:
            ...
            logging.error(f"Error displaying image with index {self._current_page_index}: {e}")
    ...
    def _next_page(self, _instance):
        self._page_manager.next_page()

    def _prev_page(self, _instance):
        self._page_manager.prev_page()

    def _wait_for_image_to_load(self):
        if self._all_loaded:
            return
    ...
    def _exit_fullscreen(self, button: ActionButton):
        if not Window.fullscreen:
            return
    ...
    def _toggle_action_bar(self) -> None:
        ...
        logging.debug(
            f"On toggle action bar entry:" f" self.action_bar.height = {self._action_bar.height}"
        )

        if self._action_bar.height <= 0.1:
            self._show_action_bar()
        else:
            self._hide_action_bar()
    ...
    def _create_goto_page_dropdown(self):
