...
inset_files_dir = _DEFAULT_PNG_BARKS_PANELS_DIR / "Insets"
comic_cover_files_dir = _DEFAULT_PNG_BARKS_PANELS_DIR / "Covers"
...
def export_inset_file(ttl: str, inset_file: Path) -> None:
    dest_file = inset_files_dir / (ttl + JPG_FILE_EXT)
...
def export_page_files_as_title_files(
    ttl: str, comic: ComicBook, dest_dir: Path, page_types: list[PageType]
) -> None:
    ...
    dest_file = dest_dir / (ttl + JPG_FILE_EXT)
...
def export_page_files_to_title_dirs(
    ...
    ttl_dir = dest_dir / ttl
    ttl_dir.mkdir(parents=True, exist_ok=True)

    for file, _is_modified in pages:
        srce_file = Path(file)
        dest_file = ttl_dir / (srce_file.stem + JPG_FILE_EXT)
...
def copy_image_file(srce: Path, dest: Path) -> None:
    image = open_pil_image_for_reading(str(srce)).convert("RGB")
